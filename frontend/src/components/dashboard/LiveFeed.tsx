...
const STATUS_MAP: Record<TokenStatus, StatusConfig> = {
  ...
  auditing: {
    ...
    rowClass: 'bg-amber-500/[0.03] border-l border-amber-500/20',
  },
  rejected: {
    dot: 'bg-red-600/70',
    label: 'REJECTED',
    labelClass: 'text-red-500/70',
    rowClass: '',
  },
  swapping: {
  ...
  return (
    ...
    >
      <div className="flex items-center space-x-2 min-w-0">
        <span className="text-[9px] text-neutral-700 w-[52px] flex-shrink-0 tabular-nums">
          {token.discoveredAt}
        </span>
        <span className="text-[10px] text-neutral-700 font-mono w-[76px] flex-shrink-0 hidden sm:block">
          {truncateAddress(token.address)}
        </span>
        <span className="text-[11px] text-neutral-300 font-semibold tracking-wide truncate">
          {token.symbol}
        </span>
      </div>
    ...
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-neutral-800 flex-shrink-0">
        <div className="flex items-center justify-between mb-1">
          ...
          <span className="text-[9px] text-neutral-700 tabular-nums">{feed.length} seen</span>
        </div>
      ...
      <div className="flex border-b border-neutral-800 flex-shrink-0">
        <div className="flex-1 px-4 py-2 border-r border-neutral-800">
          <div className="text-[9px] text-neutral-700 uppercase tracking-widest">Passed</div>
          <div className="text-sm text-white font-light tabular-nums">{passedCount}</div>
        </div>
        <div className="flex-1 px-4 py-2">
          <div className="text-[9px] text-neutral-700 uppercase tracking-widest">Rejected</div>
          <div className="text-sm text-red-500/70 font-light tabular-nums">{rejectedCount}</div>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto py-2 px-2 space-y-0.5">
        {feed.length === 0 ? (
          <div className="px-2 py-4 text-[10px] text-neutral-700 tracking-wider">
      ...
      <div className="px-4 py-2 border-t border-neutral-800 flex-shrink-0">
        <p className="text-[9px] text-neutral-800 tracking-wide">
